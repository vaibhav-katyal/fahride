...
import { Car, LogOut, User, Mail, Phone, Pencil, BookOpenCheck, ChevronRight, Camera, Star, Flag, Copy, Share2, Wallet, IndianRupee } from "lucide-react";
...
type WalletOverview = {
  ...
  daily: {
    joinRewardsUsed: number;
    joinRewardsLimit: number;
    postRewardsUsed: number;
    postRewardsLimit: number;
  };
  referral: {
    code: string;
    invitesRewarded: number;
    maxInviteRewards: number;
  };
  redemption: {
...
const transactionKindLabel: Record<string, string> = {
  signup_bonus: "Signup bonus",
  ride_post_reward: "Ride post reward",
  ride_join_reward: "Ride join reward",
  feedback_reward: "Feedback reward",
  referral_invite_bonus: "Referral invite bonus",
  referral_signup_friend_bonus: "Referral friend signup bonus",
  referral_first_join_inviter_bonus: "Referral inviter first-join bonus",
  referral_first_join_friend_bonus: "Referral friend first-join bonus",
  redemption_debit: "Redeem request",
  reversal_adjustment: "Cancellation adjustment",
};
...
const Profile = () => {
  ...
  const previousWalletBalanceRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    const loadWallet = async () => {
      ...
      setWalletError("");
      try {
        const response = await apiRequest<{ success: boolean; data: WalletOverview }>(`/wallet/me?fresh=${Date.now()}`);
        if (!isActive) return;
        ...
        if (!isActive) return;
        const message = error instanceof ApiError ? error.message : "Failed to load wallet";
  ...
  const handleProfileSave = async () => {
    ...
    try {
      const response = await apiRequest<{ success: boolean; data: { user: UserAccount } }>("/auth/me", {
        method: "PATCH",
        body: JSON.stringify({
...
setCurrentUserFromAccount({
  ...
  const referralCode = walletData?.referral.code || (currentUser as { referralCode?: string }).referralCode || "";
  const referralLink = referralCode
    ? `${window.location.origin}/signup?ref=${encodeURIComponent(referralCode)}`
    : "";
  const redeemPointsNumber = Number(redeemPoints);
  const isValidRedeemPoints = Number.isFinite(redeemPointsNumber) && redeemPointsNumber >= 500 && redeemPointsNumber % 500 === 0;
  const hasEnoughCoins = walletData ? redeemPointsNumber <= walletData.balance : false;
  const isValidUpi = upiIdRegex.test(upiId.trim());
  const canRedeem = !isRedeeming && isValidRedeemPoints && hasEnoughCoins && isValidUpi;
  ...
  const handleShareReferral = async () => {
    ...
    const shareText = `Join FahRide with my referral link: ${referralLink}`;
    if (navigator.share) {
      try {
        await navigator.share({
          title: "Join FahRide",
          text: "Sign up on FahRide using my referral link",
          url: referralLink,
        });
        return;
      } catch {
        // If native share is cancelled/unsupported, fallback to clipboard.
      }
    }

    await copyText(shareText, "Referral link copied");
  };

  const handleRedeem = async () => {
    const points = Number(redeemPoints);
    if (!Number.isFinite(points) || points < 500 || points % 500 !== 0) {
      toast.error("Redeem Fah Coins must be 500, 1000, 1500...");
      return;
    }
    ...
    setIsRedeeming(true);
    try {
      await apiRequest<{ success: boolean; message: string }>("/wallet/redeem", {
        method: "POST",
        body: JSON.stringify({
          points,
      ...
      trackEvent("redeem_points", {
          points,
          upi_id: upiId.trim().toLowerCase(),
        });
        toast.success("Redemption request submitted");
        const walletResponse = await apiRequest<{ success: boolean; data: WalletOverview }>(`/wallet/me?fresh=${Date.now()}`);
        previousWalletBalanceRef.current = walletResponse.data.balance;
        setWalletData(walletResponse.data);
        setUpiId("");
      } catch (error) {
      const message = error instanceof ApiError ? error.message : "Redemption failed";
      toast.error(message);
    } finally {
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                  <div className="rounded-xl border border-border p-2">
                    <p className="text-[11px] text-muted-foreground">Fah Coins</p>
                    <p className="text-sm font-semibold text-foreground">{walletData.balance}</p>
                  </div>
                  <div className="rounded-xl border border-border p-2">
                    <p className="text-[11px] text-muted-foreground">Weekly</p>
                    <p className="text-sm font-semibold text-foreground">{walletData.weeklyEarned}/{walletData.weeklyCap} Fah Coins</p>
                  </div>
                  <div className="rounded-xl border border-border p-2">
                    <p className="text-[11px] text-muted-foreground">Join Today</p>
                    <p className="text-sm font-semibold text-foreground">{walletData.daily.joinRewardsUsed}/{walletData.daily.joinRewardsLimit}</p>
                  </div>
                  <div className="rounded-xl border border-border p-2">
                    <p className="text-[11px] text-muted-foreground">Post Today</p>
                    <p className="text-sm font-semibold text-foreground">{walletData.daily.postRewardsUsed}/{walletData.daily.postRewardsLimit}</p>
                  </div>
                </div>

                {walletData.weeklyLimitReached ? (
                  <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2">
                    <p className="text-xs font-semibold text-amber-800">Weekly earning limit reached</p>
                    <p className="text-[11px] text-amber-700">Is hafte ab aur reward coins add nahi honge, including referral rewards.</p>
                  </div>
                ) : null}
                ...
                <div className="rounded-xl border border-border p-3">
                  <p className="text-xs font-semibold text-muted-foreground">Redeem Fah Coins (manual UPI payout)</p>
                  <div className="mt-2 grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                    <input
                      ...
                      placeholder="Fah Coins (500, 1000...)"
                    />
                    <input
                      type="text"
                      value={upiId}
                    ...
                    <button
                      ...
                      disabled={!canRedeem}
                      className="inline-flex items-center justify-center gap-1 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50"
                    >
                      <IndianRupee className="h-3.5 w-3.5" /> {isRedeeming ? "Submitting..." : "Redeem"}
                    </button>
                  </div>
                  {!isValidRedeemPoints ? (
                    <p className="mt-2 text-[11px] text-amber-700">Enter valid Fah Coins: 500, 1000, 1500...</p>
                  ) : null}
                  {isValidRedeemPoints && !hasEnoughCoins ? (
                    <p className="mt-2 text-[11px] text-amber-700">Not enough Fah Coins in wallet.</p>
                  ) : null}
                  {upiId.trim() && !isValidUpi ? (
                    <p className="mt-2 text-[11px] text-amber-700">Enter a valid UPI ID.</p>
                  ) : null}
                  <p className="mt-2 text-[11px] text-muted-foreground">
                    500 Fah Coins = Rs 50. Pending requests: {walletData.redemption.pendingRequests}
                  </p>
                </div>

                <div className="rounded-xl border border-border p-3">
                  <p className="text-xs font-semibold text-muted-foreground">Recent Fah Coins activity</p>
                        ...
                        <div key={txn.id} className="flex items-start justify-between rounded-lg border border-border/70 bg-card px-3 py-2">
                          <div className="pr-3">
                            <p className="text-xs font-medium text-foreground">
                              {transactionKindLabel[txn.kind] || txn.kind}
                            </p>
                            <p className="text-[11px] text-muted-foreground">
                              {new Date(txn.createdAt).toLocaleString()}
                            </p>
                          </div>
                          <p className={`text-xs font-semibold ${txn.pointsDelta >= 0 ? "text-emerald-700" : "text-destructive"}`}>
                            {txn.pointsDelta >= 0 ? "+" : ""}{txn.pointsDelta} Fah Coins
                          </p>
                        </div>
                      ))}
                    </div>
